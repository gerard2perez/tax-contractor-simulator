import { TaxesDatabase } from '../../support/i-taxes-table'
import { MAX_INCOME } from './max-income'
export type ServiciosProfesionalesTable  = [inferior: number, superior: number, fixed: number, rate: number]

export const TABLE = new TaxesDatabase<ServiciosProfesionalesTable>([
    [2021, [
        [0.01, 644.58, 0, 1.92],
        [644.59, 5470.92, 12.38, 6.40],
        [5470.93, 9614.66, 321.26, 10.88],
        [9614.67, 11176.62, 772.1, 16.00],
        [11176.63, 13381.47, 1022.01, 17.92],
        [13381.48, 26988.50, 1417.12, 21.36],
        [26988.51, 42537.58, 4323.58, 23.52],
        [42537.59, 81211.25, 7980.73, 30.00],
        [81211.26, 108281.67, 19582.83, 32.00],
        [108281.68, 324845.01, 28245.36, 34.00],
        [324845.02, MAX_INCOME, 101876.90, 35.00]
    ]],
    [2022, [
        [0.01, 644.58, 0, 1.92],
        [644.59, 5470.92, 12.38, 6.40],
        [5470.93, 9614.66, 321.26, 10.88],
        [9614.67, 11176.62, 772.1, 16.00],
        [11176.63, 13381.47, 1022.01, 17.92],
        [13381.48, 26988.50, 1417.12, 21.36],
        [26988.51, 42537.58, 4323.58, 23.52],
        [42537.59, 81211.25, 7980.73, 30.00],
        [81211.26, 108281.67, 19582.83, 32.00],
        [108281.68, 324845.01, 28245.36, 34.00],
        [324845.02, MAX_INCOME, 101876.90, 35.00]
    ]],
])